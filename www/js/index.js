...
function createNewToDo()
{
    ...
    if (todo != null)
    {
        ...
        else
        {
            ...
            todoDictionary = { check : 0 , text : todo, date : ""}; //this is where we store local permamenet data
...
function addTableRow(todoDictionary, appIsLoading)
{
    ...
    element5.value = todoDictionary["date"];
	element5.placeholder = "Enter date";
...
function saveToDoList()
{
    ...
    if (rowCount != 0)
    {
        ...
        for(var i=0; i<rowCount; i++)
        {
			...
			var datebox = row.cells[2].childNodes[0];
...
function checkDate(contents) {
	var parsedDate;
	var checkedDate;
	if (contents == "") {
		return "";
	}
	parsedDate = new Date(Date.parse(contents));
	checkedDate = parsedDate.toUTCString();
	return checkedDate;
}
